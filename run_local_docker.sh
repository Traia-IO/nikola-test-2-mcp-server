...
# Script to build and run the Nikola Test 2 MCP Server locally in Docker
...
IMAGE_NAME="nikola-test-2-mcp-server"
CONTAINER_NAME="nikola-test-2-mcp-local"
HOST_PORT=${PORT:-8000}
CONTAINER_PORT=${PORT:-8000}

echo -e "${BLUE}🚀 Building and running Nikola Test 2 MCP Server...${NC}"
echo
...
if [ ! -f .env ]; then
    ...
    if [ -f .env.example ]; then
        ...
        cat > .env << EOF
# Nikola Test 2 MCP Server Configuration
PORT=8000
STAGE=MAINNET
LOG_LEVEL=INFO
...
echo
echo -e "${GREEN}✅ Nikola Test 2 MCP Server is running!${NC}"
echo
