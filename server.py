#!/usr/bin/env python3
"""
Nikola Test 2 MCP Server - FastMCP with D402 Transport Wrapper
...
logger = logging.getLogger('nikola-test-2_mcp')
...
logger.info("="*80)
logger.info(f"Nikola Test 2 MCP Server (FastMCP + D402 Wrapper)")
logger.info(f"API: https://petstore.swagger.io/")
logger.info(f"Payment: {SERVER_ADDRESS}")
logger.info("="*80)

# Create FastMCP server
mcp = FastMCP("Nikola Test 2 MCP Server", host="0.0.0.0")
...
def create_app_with_middleware():
    ...
    app.add_middleware(
        ...
        server_name="nikola-test-2-mcp-server"  # MCP server ID for tracking
    )
    ...
    async def health_check(request: Request) -> JSONResponse:
        ...
        return JSONResponse(
            content={
                "status": "healthy",
                "service": "nikola-test-2-mcp-server",
...
if __name__ == "__main__":
    logger.info("="*80)
    logger.info(f"Starting Nikola Test 2 MCP Server")
    logger.info("="*80)
